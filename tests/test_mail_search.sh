...
"$PROBE_PATH" call microsoft365_mail_search \
  --server "$SERVER_URL" \
...
"$PROBE_PATH" call microsoft365_mail_search \
  --server "$SERVER_URL" \
  --headers "Authorization:Bearer $APIKEY" \
  --\$filter "contains(from/emailAddress/address,'@microsoft.com')" \
...
"$PROBE_PATH" call microsoft365_mail_search \
  --server "$SERVER_URL" \
  --headers "Authorization:Bearer $APIKEY" \
  --\$filter "contains(subject,'Project') and receivedDateTime ge 2025-01-01T00:00:00Z" \
...
"$PROBE_PATH" call microsoft365_mail_search \
  --server "$SERVER_URL" \
  --headers "Authorization:Bearer $APIKEY" \
  --\$filter "contains(bodyPreview,'urgent') or contains(subject,'urgent')" \
...
"$PROBE_PATH" call microsoft365_mail_search \
  --server "$SERVER_URL" \
  --headers "Authorization:Bearer $APIKEY" \
  --\$filter "toRecipients/any(r:contains(r/emailAddress/address,'team'))" \
  --\$select "subject,toRecipients,from" \
  --\$top 5 \
  | tee -a "$OUTPUT_FILE"
echo | tee -a "$OUTPUT_FILE"
...
"$PROBE_PATH" call microsoft365_mail_search \
  --server "$SERVER_URL" \
  --headers "Authorization:Bearer $APIKEY" \
  --\$search "attachment:*.pdf" \
  --\$select "subject,hasAttachments,from" \
  --\$top 5 \
  | tee -a "$OUTPUT_FILE"
echo | tee -a "$OUTPUT_FILE"
