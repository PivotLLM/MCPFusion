...
# Test 1: List Documents folder
echo "=== Test 1: List Documents Folder ===" | tee -a "$OUTPUT_FILE"
"$PROBE_PATH" -url "$FULL_SERVER_URL" -transport http \
  -headers "Authorization:Bearer $APIKEY" \
  -call microsoft365_files_list_folder_by_path \
  -params '{"folderPath": "Documents", "$top": 5, "$orderby": "lastModifiedDateTime desc"}' \
  2>&1 | tee -a "$OUTPUT_FILE"
echo | tee -a "$OUTPUT_FILE"

# Test 2: Non-existent folder (error handling)
echo "=== Test 2: Non-Existent Folder (Error Test) ===" | tee -a "$OUTPUT_FILE"
"$PROBE_PATH" -url "$FULL_SERVER_URL" -transport http \
  -headers "Authorization:Bearer $APIKEY" \
  -call microsoft365_files_list_folder_by_path \
