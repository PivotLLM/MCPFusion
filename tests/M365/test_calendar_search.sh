...
END_DATE=$(date -v+30d '+%Y%m%d' 2>/dev/null || date -d "+30 days" '+%Y%m%d')

echo "Date range: $START_DATE to $END_DATE" | tee -a "$OUTPUT_FILE"
echo | tee -a "$OUTPUT_FILE"

# Test 1: Search by subject keyword
...
"$PROBE_PATH" -url "$FULL_SERVER_URL" -transport http -headers "Authorization:Bearer $APIKEY" \
  -call microsoft365_calendar_search \
  -params "{\"startDate\": \"$START_DATE\", \"endDate\": \"$END_DATE\", \"\$filter\": \"contains(subject,'Meeting')\", \"\$top\": \"5\"}" \
  | tee -a "$OUTPUT_FILE"
echo | tee -a "$OUTPUT_FILE"

# Test 2: All events in date range (no filter)
echo "=== Test 2: All events in date range ===" | tee -a "$OUTPUT_FILE"
"$PROBE_PATH" -url "$FULL_SERVER_URL" -transport http -headers "Authorization:Bearer $APIKEY" \
  -call microsoft365_calendar_search \
  -params "{\"startDate\": \"$START_DATE\", \"endDate\": \"$END_DATE\", \"\$top\": \"5\"}" \
  | tee -a "$OUTPUT_FILE"
echo | tee -a "$OUTPUT_FILE"
