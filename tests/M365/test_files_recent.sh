...
# Test 1: List recent files with count limit
...
# Test 2: List recent files with custom field selection
echo "=== Test 2: List Recent Files (Custom Fields) ===" | tee -a "$OUTPUT_FILE"
"$PROBE_PATH" -url "$FULL_SERVER_URL" -transport http \
  -headers "Authorization:Bearer $APIKEY" \
  -call microsoft365_files_recent \
  -params '{"$select": "name,webUrl,lastModifiedDateTime,size", "$top": 5}' \
  2>&1 | tee -a "$OUTPUT_FILE"
echo | tee -a "$OUTPUT_FILE"
